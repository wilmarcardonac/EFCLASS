...
int perturb_total_stress_energy(
  ...
  double w_fld,dw_over_da_fld,integral_fld,ca2;
  double pi_fld;
  double shear_fld=0.;
  double gwncdm;
  ...
  if (_scalars_) {
    ...
    */
	...
	pi_fld = ppt->e_pi*(y[ppw->pv->index_pt_delta_cdm] + 3.*a_prime_over_a*y[ppw->pv->index_pt_theta_cdm]/k2 )
	  + ppt->f_pi*( y[ppw->pv->index_pt_delta_fld] + 3.*a_prime_over_a*(1.+w_fld)*y[ppw->pv->index_pt_theta_fld]/k2  )/(1. 
	  + ppt->g_pi*ppt->g_pi*a_prime_over_a*a_prime_over_a/k2) ; 
	shear_fld = 2.*pi_fld/3./(1.+w_fld);
	ca2 = w_fld - a * dw_over_da_fld / 3. / (1.+w_fld) ;
	...
	if (ppt->Anad != 0.) 
	  {
	    ppw->delta_p += (ca2 + ppt->Anad*pow(a,ppt->Nnad))*ppw->delta_rho_fld;
	  }
	else
	  {
	    ppw->delta_p += pba->cs2_fld * ppw->delta_rho_fld  
       	+ 3.*(1.+w_fld)*(pba->cs2_fld - ca2)*ppw->pvecback[pba->index_bg_rho_fld]*a_prime_over_a*y[ppw->pv->index_pt_theta_fld]/k2;
	  }
	ppw->rho_plus_p_shear = ppw->rho_plus_p_shear  + (1.+w_fld)*ppw->pvecback[pba->index_bg_rho_fld]*shear_fld;
      }
      else {
...
int perturb_derivs(double tau,
  ...
  if (_scalars_) {
	...
	if (ppt->Anad != 0.)
	  {
	    cs2 = ca2 + ppt->Anad*pow(a,ppt->Nnad);
	  }
	else
	  {
	    cs2 = pba->cs2_fld;
	...
	pi_fld = ppt->e_pi*(y[pv->index_pt_delta_cdm] + 3.*a_prime_over_a*y[pv->index_pt_theta_cdm]/k2 ) 
	         + ppt->f_pi*( y[pv->index_pt_delta_fld] + 3.*a_prime_over_a*(1.+w_fld)*y[pv->index_pt_theta_fld]/k2  )/(1. 
	         + ppt->g_pi*ppt->g_pi*a_prime_over_a*a_prime_over_a/k2) ;
